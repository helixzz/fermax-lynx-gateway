...
async function logs(older){loadedOlder=older;const q=new URLSearchParams({limit:'50',kind:$('#kind').value});if(older&&before)q.set('before',before);const result=await api(($('#kind').value==='diagnostics'?'/v1/diagnostics?':'/v1/logs?')+q);if(!older)$('#events').replaceChildren();for(const e of result.events){const tr=document.createElement('tr');for(const value of [new Date(e.time*1000).toLocaleString('zh-CN'),e.text || e.kind]){const td=document.createElement('td');td.textContent=value;tr.append(td)}const td=document.createElement('td'),details=document.createElement('details'),summary=document.createElement('summary'),pre=document.createElement('pre');summary.textContent=e.kind;pre.textContent=JSON.stringify(e.detail,null,2);details.append(summary,pre);td.append(details);tr.append(td);$('#events').append(tr)}before=result.next_before;$('#older').disabled=result.events.length<50}
